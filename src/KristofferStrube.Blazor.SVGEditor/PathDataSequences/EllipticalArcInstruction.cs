...
public class EllipticalArcInstruction : BasePathInstruction
{
    ...
    public (double x, double y) Center
    {
        get
        {
            ...
            double sqrtPart = Delta <= 1
                ? sign * Math.Sqrt(((rxPow * ryPow) - (rxPow * y1MarkPow) - (ryPow * x1MarkPow)) / ((rxPow * y1MarkPow) + (ryPow * x1MarkPow)))
                : 0;
    ...
    {
        get => Delta <= 1 ? ((double Rx, double Ry))(Math.Abs(Rx), Math.Abs(Ry)) : ((double Rx, double Ry))(Math.Sqrt(Delta) * Math.Abs(Rx), Math.Sqrt(Delta) * Math.Abs(Ry));
        set
        {
            if (Delta <= 1)
            {
